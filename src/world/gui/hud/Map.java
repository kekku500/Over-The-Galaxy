...
public class Map extends AbstractComponent {
	...
	public Map(HUDManager hudManager, Player player){
		...
		PLoc = new Vector3f(player.getTransform(RenderState.getUpdatingId()).origin);
		setPosition(200,50);
	}
	...
	public void update(float dt) {
		if(render){
			Entitys = getHUDManager().getState().getEntityManager().getVisualEntities(RenderState.getUpdatingId());
			PLoc = new Vector3f(getHUDManager().getState().getCamera().getTransform(RenderState.getUpdatingId()).origin);
			elemente=0;
