...
public class Request<T>{
	...
	public enum Status{
		...
		CONTINUE,
		FIRST} //Request not done yet (more world states to update)
	...
	public Status requestStatus(){
		if(updatedWorlds.contains(RenderState.updating())){
			return Status.IDLE;
		}else
			if(updatedWorlds.size() >= 2)
				return Status.FINAL;	
		updatedWorlds.add(RenderState.updating());
		if(updatedWorlds.size() == 1)
			return Status.FIRST;
